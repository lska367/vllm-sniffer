...
| [EVENT_SCHEMA.md](EVENT_SCHEMA.md) | 事件类型的完整字段语义 + JSONL 示例（含 `env_snapshot` 参照系、`logits_fp` 位级指纹） | 写分析工具 / 前端 / 新 hook 时 |
| [EXTENSION_ROADMAP.md](EXTENSION_ROADMAP.md) | 延伸拓展路线图：按求职价值/科研价值/平台化分优先级 | 决定"下一步做什么"时 |
| [WEBAPP.md](WEBAPP.md) | 可视化前端：启动方式、聚合接口结构、前端实现、扩展指南 | 用前端 / 加视图时 |
...
> latency_report / repro_compare / logits_fp_compare，README 有用法）。
...
- `README.md`（根目录）：用户视角，安装/配置/事件一览（英文版）
